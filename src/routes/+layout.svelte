<script>
	...
	// Self-hosted variable fonts (Archivo wdth+wght, Roboto, Roboto Mono).
		...
		await new Promise((resolve) => {
			...
			const onTransitionEnd = (event) => {
				if (
					...
					(event.propertyName === 'grid-template-rows' || event.propertyName === 'visibility')
				) {
					finish();
				}
			};
					...
					<ul class="navbar-nav ms-auto">
						...
						<li class="nav-item dropdown">
							<button
								...
								class:active={isActive('/utility')}
								type="button"
								aria-expanded={utilityOpen}
								aria-haspopup="true"
								aria-controls="utilityMenu"
								id="utilityDropdown"
							...
							<ul
								class="dropdown-menu dropdown-menu-dark dropdown-menu-end utility-dropdown"
								class:show={utilityOpen}
								id="utilityMenu"
						...
						<li class="nav-item">
							<a
								...
								href="https://discord.gg/RSp2CSuMny"
								target="_blank"
						...
						<ul class="list-unstyled mb-0 footer-links">
							...
							<li>
								<a
									...
									target="_blank"
									rel="noopener noreferrer"
									use:tooltip={{ content: 'Join Discord Server', placement: 'right', theme: primaryTooltipTheme }}
									>Discord</a
								>
							</li>
							<li>
								<a
									href="mailto:anonymousgca@anonymousgca.eu"
									use:tooltip={{ content: 'Send Email', placement: 'right', theme: primaryTooltipTheme }}>Email</a
								>
							</li>
							<li>
								<a
									href="https://www.linkedin.com/in/gabriele-caretti-046408270/"
									target="_blank"
									rel="noopener noreferrer"
									use:tooltip={{ content: 'View LinkedIn Profile', placement: 'right', theme: primaryTooltipTheme }}
									>Linkedin</a
								>
							</li>
						</ul>
...
<style>
	...
	.marquee__track {
		display: flex;
		width: max-content;
		/* Promoted once — the band loops forever, so keep it off the
           main thread's repaint path for the whole session. */
		will-change: transform;
	...
	@media (max-width: 991.98px) {
		.navbar-collapse.collapse {
			...
			opacity: 0;
			transition:
				grid-template-rows 0.35s var(--ease-out-expo),
				opacity 0.2s ease,

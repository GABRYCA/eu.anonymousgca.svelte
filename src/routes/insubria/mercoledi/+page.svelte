<script>
	...
	function attesaTraMezzi(viaggio, indice) {
		...
		let arrivoPrimo = new Date(
			2000,
			0,
			1,
			viaggio[indice].oraArrivo.split(':')[0],
			viaggio[indice].oraArrivo.split(':')[1]
		);
		let partenzaSecondo = new Date(
			2000,
			0,
			1,
			viaggio[indice + 1].oraPartenza.split(':')[0],
			viaggio[indice + 1].oraPartenza.split(':')[1]
		);
	...
	function durataMezzo(viaggio, indice) {
		...
		let partenza = new Date(
			2000,
			0,
			1,
			viaggio[indice].oraPartenza.split(':')[0],
			viaggio[indice].oraPartenza.split(':')[1]
		);
...
<div class="row text-center rounded-5 border border-light-subtle pt-5 pb-5 mx-1 mb-4 shadow">
	<div class="col text-white">
		...
		<p class="h3 text-danger-emphasis bg-light-subtle bg-opacity-25 pt-1 pb-2 rounded-4 mt-3">
			Nessuna lezione? <span class="text-primary-emphasis">Per ora...</span>
		</p>
		...
		<p class="h3 text-danger-emphasis bg-light-subtle bg-opacity-25 pt-1 pb-2 rounded-4 mt-4">
			Sul momento, <span class="text-primary-emphasis">ormai abbiamo imparato!</span>
		</p>
		...
		<div class="row justify-content-center">
			<div class="col-auto">
				<h2 class="shadow px-5 pt-1 pb-1 rounded bg-primary bg-opacity-10">
					Anticipatario Andata <strong class="text-danger-emphasis">(NO)</strong>:
				</h2>
			</div>
		</div>
		...
		<table class="table table-hover table-striped shadow mt-3">
			...
			<tbody>
				{#each viaggio.andata.anticipatario as mezzo, i}
					<tr>
						<td><i class="fa fa-{mezzo.icona}"></i> {mezzo.nome}</td>
						<td use:tooltip={{ content: mezzo.luogoPartenza, placement: 'top', theme: darkTooltipTheme }}
							>{mezzo.oraPartenza}</td
						>
						<td use:tooltip={{ content: mezzo.luogoArrivo, placement: 'top', theme: darkTooltipTheme }}
							>{mezzo.oraArrivo}</td
						>
		...
		<div class="row justify-content-center mt-4">
			<div class="col-auto">
				<h2 class="shadow px-5 pt-1 pb-1 rounded bg-primary bg-opacity-10">
					Anticipatario Ritorno <strong class="text-danger-emphasis">(NO)</strong>:
				</h2>
			</div>
		</div>

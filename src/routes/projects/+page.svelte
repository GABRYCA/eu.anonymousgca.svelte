<script>
	...
	const filtered = $derived(
		activeKind === 'all' ? data.projects : data.projects.filter((project) => project.kind === activeKind)
	);
	...
	function kindCount(kindId) {
		return kindId === 'all' ? data.projects.length : data.projects.filter((project) => project.kind === kindId).length;
	}
...
<div class="projects-page">
	<div class="container-xxl">
		<section class="projects-hero" use:scrollAnimation={{ animation: 'ink-up', duration: 700 }}>
			...
			<p class="projects-hero__lead">
				Live websites, Svelte packages and components, university work and developer tooling. Everything here is shipped
				and open, sorted by most recent activity — pick a category and dive in.
			</p>
		</section>
	</div>
	...
	<section class="catalogue" aria-labelledby="catalogue-heading">
		<div class="container-xxl">
					...
					{#each filtered as project, i (project.url)}
						...
						<article
							...
							in:fly={{
								...
								delay: reducedMotion ? 0 : Math.min(i * 40, 280)
							}}
						>
							...
							<div class="project-card__actions">
								...
								{/if}
								<a class="btn-plate btn-plate--solid" href={project.url} target="_blank" rel="noopener noreferrer">
									{primaryCta(project)}
			...
			<a
				class="github-row"
				href="https://github.com/GABRYCA?tab=repositories"
				target="_blank"
